...
def displayEpipolarF(I1, I2, F):
    ...
    while True:
        plt.sca(ax1)
        x,y=plt.ginput(1,mouse_stop=3)[0]
...
def _singularize(F): #I made changes to S matrix
    U, S, V= np.linalg.svd(F)

    S[-1] = 0
    S=np.diag(S) #9x9 matrix
    S= np.concatenate((S, np.zeros((int(U.shape[0])-9,9))), axis=0) #made it Nx9 matrix
